...
log_un_eta<-function(index,theta_vec,label_vec,gamma,scheme){
  ...
  if(scheme=="PG"){
    ...
    log_den<-gamma*(log_un_pi(theta_vec[index,])-logSumExp(log_q_cond_matL(theta_vec[index,],label_vec)))+
      sum(log_q_mat(theta_vec,label_vec))-(1-gamma)*log(length(theta_vec))
  }
  else if(scheme=="SG"){
    
    log_den<-gamma*log_un_pi(theta_vec[index])-logSumExp(gamma*log_q_cond_matL(theta_vec[index],label_vec))+
...
log_un_pi<-function(theta){
  ...
  #return(dnorm(theta,mu_pi,sd_pi,log=T)+lZ_true)
  return(dmvn(theta,mu_vec_pi,Cov_pi,log=T)+lZ_true)
}
...
log_alpha<-function(l){
  ...
  if(s==Inf)
    ...
    log_den_l<-rep(-log(K),d)
  else
    log_den_l<-lbeta(l-1+s*m,K-1-(l-1)+s*(1-m)) + lchoose(K-1,l-1) -lbeta(s*m,s*(1-m))
  #log_den_l<-dbetabinom(l-1,K-1,m,s,log=T)
  
  return(sum(log_den_l))
}

log_alpha_mat<-function(l){
  
  d<-ncol(l)
  N<-nrow(l)
  
  log_den_l<-sapply(1:N, function(i) log_alpha(l[i,]))
...
log_q_cond<-function(theta,l){
  ...
  mu_prop<-(mu_prop_max-mu_prop_min)*(l-1)/max(K-1,1)+mu_prop_min
  log_den_theta<-sum(dnorm(theta,mu_prop,diag(Cov_prop),log=T))
  
  return(log_den_theta)
}

log_q_cond_matL<-function(theta,l){
  
  d<-ncol(l)
  n<-nrow(l)
  
  mu_prop<-(mu_prop_max-mu_prop_min)*(l-1)/max(K-1,1)+mu_prop_min
  log_den_theta<-sapply(1:n,function(i) sum(dnorm(theta,mu_prop[i,],diag(Cov_prop),log=T)))
...
log_q_mat<-function(theta,l){
  
  return(log_q_cond(theta,l)+log_alpha_mat(l))
}

log_q_unif<-function(theta){
  
  return(-d*log(mu_prop_max-mu_prop_min))
}

log_q_marg<-function(theta){
  
  I <- iterpc(K,d,ordered=TRUE,replace=TRUE)
  perms<-getall(I)
  
  log_den<-sapply(1:nrow(perms),function(i){
    
    log_q(theta,perms[i,])
  })
  
  return(logSumExp(log_den))
}

r_q<-function(N){ #Sample is a Matrix of N rows, 1st column is label
  
  if(s==Inf)
    lvec<-rbinom(N*d,K-1,m)+1
  else
    lvec<-rbinom(N*d,K-1,rbeta(N*d,s*m,s*(1-m)))+1
  
  l<-matrix(lvec,ncol=d,byrow=T)
  mu_prop<-(mu_prop_max-mu_prop_min)*(l-1)/max(K-1,1)+mu_prop_min
  
  theta<-rmvn(N,mu_prop,Cov_prop)
  
  return(cbind(l,theta))
}
...
r_q_cond<-function(l){
  ...
  theta<-rmvn(1,mu_prop,Cov_prop)
  
  return(theta)
}

r_q_unif<-function(N){ #Sample is a Matrix of N rows, 1st column is label
  
  theta<-runif(N*d,min=mu_prop_min,max=mu_prop_max)
  
  return(cbind(matrix(1,nrow=N,ncol=d),matrix(theta,ncol=d)))
}
...
altRep<-function(Particles,lW=NA){ #Sample is a List of size K_eff, each element also a list of size 2: 1st element is label, 2nd is vector of values
  ...
  N<-nrow(Particles)
  labels<-unique(Particles[,1:d])
  if(max(is.na(lW))){
    
    result<-lapply(1:nrow(labels), function(i){
      
      ind_vec<-which(rowSums(abs(Particles[,1:d]-matrix(labels[i,],nrow=N,ncol=d,byrow=T)))==0)
      return(list(labels[i,],matrix(Particles[ind_vec,(d+1):(2*d)],nrow=length(ind_vec),ncol=d)))
    })
  }
  # else{
  #   
  #   result<-lapply(1:nrow(labels), function(i){
  #     
  #     ind_vec<-which(rowSums(Particles[,1:d]==labels[i,])==d)
  #     PartsNlW<-cbind(Particles,lW[])[ind_vec,(d+1):(2*d+1)]
  #     
...
obtain_Nj<-function(Particles_alt){ #Compute vector of number of values for each label
  ...
  N_j<-sapply(1:K_eff, function(k) NROW(Particles_alt[[k]][[2]]) )
...
IS_opt_AIS<-function(gamma_vec,Particles,MCMCtype="none",iters=1,verbose=F){
  ...
  lW<-log_un_pi(Particles[,(d+1):(2*d)])-sapply(1:N, function(i) log_q_marg(Particles[i,(d+1):(2*d)]))
...
IS_unif_AIS<-function(gamma_vec,Particles,MCMCtype="none",iters=1,verbose=F){
  
  N<-nrow(Particles)
  AIS_steps<-length(gamma_vec)
  
  lW<-log_un_pi(Particles[,(d+1):(2*d)])-sapply(1:N, function(i) log_q_unif(Particles[i,(d+1):(2*d)]))
...
IS_BH_AIS<-function(gamma_vec,Particles,Particles_alt,MCMCtype="none",iters=1,verbose=F){
  ...
  lW<-sapply(1:N, function(i) log_un_pi(Particles[i,(d+1):(2*d)])-logSumExp(log(N_j)+log_q_cond_matL(Particles[i,(d+1):(2*d)],labels)))
...
BH_AIS_std<-function(gamma_vec,Particles,Particles_alt,scheme,MCMCtype="none",iters=1,verbose=F,sigma_prop_MCMC){
  ...
  lW<-logSumExp(sapply(1:N, function(i) log_un_eta(i,Particles[,(d+1):(2*d)],Particles[,1:d],gamma_vec[1],scheme))) -
    logSumExp(sapply(1:N, function(i) log_un_eta(i,Particles[,(d+1):(2*d)],Particles[,1:d],0,scheme)))
  ...
  for(t in 1:(AIS_steps-1)){
    ...
    lW[t+1]<-logSumExp(sapply(1:N, function(i) log_un_eta(i,Particles[,(d+1):(2*d)],Particles[,1:d],gamma_vec[t+1],scheme))) -
      logSumExp(sapply(1:N, function(i) log_un_eta(i,Particles[,(d+1):(2*d)],Particles[,1:d],gamma_vec[t],scheme)))
...
BH_AIS_mod<-function(gamma_vec,Particles,Particles_alt,scheme,MCMCtype="none",iters=1,verbose=F,sigma_prop_MCMC){
  ...
  lW<-sapply(1:N, function(i) log_un_eta(i,Particles[,(d+1):(2*d)],Particles[,1:d],gamma_vec[1],scheme)) -
    logSumExp(sapply(1:N, function(i) log_un_eta(i,Particles[,(d+1):(2*d)],Particles[,1:d],0,scheme)))
